  ...
  return(
    <div>
      {/* <pre>{JSON.stringify(postMessage,null,4)}</pre> */}
        ...
        <div>
            {posts.slice(null, 4).map(post => {
              return <div key={post.id} className="rounded-4xl m-auto w-70 flex flex-col my-2 border-b-gray-700 border-4 p-6 bg-gray-300">
                ...
                <p>{post.body}</p>
                <p>{post.title}</p>
                <p><span className="font-bold">@mark </span>{post.comment}</p>
              </div>
            })}
        </div>
    </div>
